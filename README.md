...
| GET | `/api/settings` | Ingest interval, **`ebay_search_limit`**, **`ingest_max_pages`**, stale-refresh toggles (**`stale_listing_refresh_*`**), **`match_queue_sync_interval_minutes`** (0–1440; **0** = scheduler off), **`watch_catalog_queue_require_identity`** (queue only rows with brand + reference/family in **review** mode), saved Browse query lines, **`watch_catalog_excluded_brands`**, optional **Everywatch** **`everywatch_login_email`** + **`everywatch_password_configured`** (password never returned), env fallback hint |
| PATCH | `/api/settings` | Same fields as GET: interval, limits, pages, ingest lines, **`watch_catalog_review_mode`**, **`watch_catalog_queue_require_identity`**, **`watch_catalog_excluded_brands`**, **`everywatch_login_email`**, **`everywatch_login_password`** (omit to keep; **`""`** clears), **`stale_listing_refresh_enabled`**, interval/max-per-run/min-age for stale batch, **`match_queue_sync_interval_minutes`** |
...
| POST | `/api/ingest/active-refresh-all-run` | Queue a full pass over **all** active listings (adaptive pacing; live progress via status endpoint) |
| GET | `/api/ingest/active-refresh-all-status` | Poll progress for full active refresh (running, item X of Y, last status/error) |
...
- **Refresh paths** (**listing refresh**, **stale batch**, **full active refresh**) combine **Browse `getItem`** when available (e.g. **`listing_ended_at`** vs now) with **HTML checks** on the item page (multiple eBay hostnames, retries on **5xx**). **`is_active`** is set **`false`** when the page shows **not-found** markers (e.g. **“We looked everywhere”**), a **sold** banner (**“This listing sold on …”**), or **`getItem`** clearly indicates the listing has ended. If the API omits the item and the HTML state is **ambiguous**, the prior **`is_active`** value is kept to avoid false inactive flips. Ingest search alone does **not** mark missing rows inactive (an item can leave your result set but still be live).
...
- **Ingest now:** Calls **`POST /api/ingest/run`** (background task). **Stale refresh now** calls **`POST /api/ingest/stale-refresh-run`**. **Refresh ALL active now** (Settings) calls **`POST /api/ingest/active-refresh-all-run`** and polls **`GET /api/ingest/active-refresh-all-status`**. There is **no authentication** on these endpoints — intended for trusted LAN / self-hosted use only.
