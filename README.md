...
| POST | `/api/watch-models/{uuid}/refresh-market-snapshots` | Fetches Everywatch model page(s) + attempts Chrono24 search; stores **`market_source_snapshots`** JSONB. If **manual £ low/high** are missing, seeds them from Everywatch **GBP** values when present (detail **`price_analysis`** / listing hit amounts); otherwise falls back to **median** non-GBP → GBP (Frankfurter) with a ±10% band. **`EXTRA_MARKET_IMPORT_ENABLED=false`** returns failure. Respects **`MARKET_SNAPSHOT_COOLDOWN_HOURS`** only for automatic runs; this POST is **force** refresh |
