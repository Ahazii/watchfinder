...
| GET | `/api/settings` | Ingest interval, **`ebay_search_limit`**, **`ingest_max_pages`**, stale-refresh toggles (**`stale_listing_refresh_*`**), **`match_queue_sync_interval_minutes`** (0–1440; **0** = scheduler off), **`watch_catalog_queue_require_identity`** (queue only rows with brand + reference/family in **review** mode), saved Browse query lines, **`watch_catalog_excluded_brands`** (used as excluded listing words/phrases), optional **Everywatch** **`everywatch_login_email`** + **`everywatch_password_configured`** (password never returned), env fallback hint |
| PATCH | `/api/settings` | Same fields as GET: interval, limits, pages, ingest lines, **`watch_catalog_review_mode`**, **`watch_catalog_queue_require_identity`**, **`watch_catalog_excluded_brands`** (excluded listing words/phrases), **`everywatch_login_email`**, **`everywatch_login_password`** (omit to keep; **`""`** clears), **`stale_listing_refresh_enabled`**, interval/max-per-run/min-age for stale batch, **`match_queue_sync_interval_minutes`** |
...
| POST | `/api/ingest/apply-excluded-words-all` | Scan all listings and mark inactive rows matching excluded words/phrases from Settings/env |
...
- **Ingest now:** Calls **`POST /api/ingest/run`** (background task). **Stale refresh now** calls **`POST /api/ingest/stale-refresh-run`**. **Refresh ALL active now** (Settings) calls **`POST /api/ingest/active-refresh-all-run`** and polls **`GET /api/ingest/active-refresh-all-status`**. The full active pass intentionally avoids per-item HTML marker fetches for throughput/reliability. There is **no authentication** on these endpoints — intended for trusted LAN / self-hosted use only.
