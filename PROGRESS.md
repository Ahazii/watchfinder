...
- **Active-state refinement (Apr 2026):** refresh paths combine **Browse `getItem`** (e.g. **`listing_ended_at`**) with **HTML** checks on item pages (multiple **`/itm/{id}`** URLs, retries on **5xx**): **not-found** markers such as **“We looked everywhere”** → inactive; **sold** banner text (**“This listing sold on …”**) → inactive; when **`getItem`** is unavailable and page state is **unknown**, **`is_active`** is left unchanged to limit false flips.
...
- **Pagination controls (Apr 2026):** **`/listings/`**, **`/candidates/`**, and **`/not-interested/`** now include user-selectable rows-per-page controls (persisted in localStorage) with page navigation based on the selected size.
- **Not interested workflow (Apr 2026):** new **`not_interested_listings`** table + migration **010**, API (`/api/not-interested`, restore/delete), and UI page **`/not-interested/`**. Buttons on listings list/detail and match queue list/detail mark rows as **Not interested** (remove listing + block eBay item id). Ingest skips active blocklist ids until restored.
- **Full active refresh (Apr 2026):** **`POST /api/ingest/active-refresh-all-run`** re-checks every active listing with adaptive delay/jitter/backoff; **`GET /api/ingest/active-refresh-all-status`** drives live progress on **Settings** (**Refresh ALL active now**).
- **Full active refresh speed-up (Apr 2026):** bulk active refresh now skips per-item eBay HTML marker checks and relies on Browse signals to avoid long stalls from repeated **403/503** page responses.
- **Excluded listing words/phrases (Apr 2026):** Settings field **`watch_catalog_excluded_brands`** now drives whole-word, case-insensitive exclusion terms across listing content. Matching rows are forced inactive and prevented from reactivation during ingest/refresh. Added **`POST /api/ingest/apply-excluded-words-all`** for retroactive scan + inactivation with summary counts.
- **Schema** migrations **005**–**010**: **`ebay_item_id`** width **128**; **`watch_models`** spec columns + **`reference_url`**; **`external_price_history`** (JSONB) + **`watchbase_imported_at`**; **`008`** **`market_source_snapshots`** (Everywatch/Chrono24 JSON); **`009`** **`everywatch_url`** (optional exact Everywatch listing page for snapshots / **`GET /api/market/search`**); **`010`** **`not_interested_listings`** (blocklist by **`ebay_item_id`** + soft history).
...
- **Queue identity gate (2026):** **`watch_catalog_queue_require_identity`** in **`app_settings`** (Settings + Match queue toggle). When **true** (default), in **`review`** mode only listings with parseable **brand + (reference or family)** enqueue; when **false**, identity-thin rows may still queue.
...
| `backend/watchfinder/api/` | Routes, **`ingest_admin.py`** (ingest / stale / **active-refresh-all**), **`listing_detail.py`**, **`listing_sort.py`**, **`market.py`**, **`everywatch_debug.py`** (**`/api/everywatch/debug`**), deps, query |
